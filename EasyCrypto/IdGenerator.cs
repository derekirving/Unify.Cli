...
namespace Unify.Cli.EasyCrypto
{
    ...
    {
        private static readonly Random Rand = new Random(GetSeed());
        private static readonly CryptoRandom CryptoRand = new CryptoRandom();
        ...
        private string GetRandomPart()
        {
            ...
            if (FastRandom)
            {
                ...
                {
                    index = Rand.Next(SystemStringBase55Converter.Charset.Length);
                    ret[i] = SystemStringBase55Converter.Charset[Convert.ToInt16(index)];
                }
            }
            else
            {
                for (int i = 0; i < ret.Length; i++)
                {
                    index = CryptoRand.NextLong(SystemStringBase55Converter.Charset.Length);
                    ret[i] = SystemStringBase55Converter.Charset[Convert.ToInt16(index)];
                }
            }
